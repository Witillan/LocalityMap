...
import ValidationNewFornecedorSchema from '../../../components/Validation/NewFornecedorValidation'
import CidadeDao from '../../../db/CidadeDao'
import FornecedorDAO, { TipoPessoa } from '../../../db/FornecedorDao'
...
const NewFornecedorScreen = () => {
  ...
  const disabled = useMemo(() => {
    if (tempId) {
      return !!config.cadastrarClientes
    } else {
      return !!config.alterarClientes
    }
  }, [config, tempId])
  ...
  }, [changeCep, isInternetReachable])

  const confirmarCpfOrCnpj = async (cpfOrCnpj) => {
    if (isInternetReachable) {
      return getRequestOptions('GET', true)
        .then(async (options) => fetch(`${await getApiUrl()}/Fornecedores/VerificarCpfOuCnpj?cpfOuCnpj=${cpfOrCnpj}`, options))
        .then((r) => {
          if (!r.ok) {
  ...
  useFocusEffect(useCallback(() => {
    ...
    const init = async () => {
      ...
      const fornecedores = await FornecedorDAO.GetFornecedorWithTempId(tempId)
      const cidade = await CidadeDao.GetByCodigoIbge(fornecedores.codigoIbgeCidade)

      setTipoPessoa(fornecedores.tipoPessoa)
      setId(fornecedores.id)
      setIdAlphaExpress(fornecedores.idAlphaExpress)
      setNomeRazao(fornecedores.nomeRazao)
      setApelido(fornecedores.apelido)
      setRg(fornecedores.rg)
      setCep(fornecedores.cep)
      setTelefone(fornecedores.telefone)
      setCelular(fornecedores.celular)
      setContato(fornecedores.contato)
      setUf(cidade.uf)
      setCodigoIbgeCidade(fornecedores.codigoIbgeCidade)
      setObservacao(fornecedores.observacao)
      setEndereco(fornecedores.endereco)
      setNumero(fornecedores.numero)
      setBairro(fornecedores.bairro)
      setComplemento(fornecedores.complemento)
      setVendedores(fornecedores.vendedores)

      if (fornecedores.tipoPessoa === TipoPessoa.PessoaFisica) {
        setCpf(fornecedores.cpf)
      } else if (fornecedores.tipoPessoa === TipoPessoa.PessoaJuridica) {
        setCnpj(fornecedores.cpf)
      }
    }
    ...
    ValidationNewFornecedorSchema
      ...
      .then(async () => {
        try {
          ...
          await FornecedorDAO.Insert(obj)

          if (isInternetReachable) {
            setMessage('Sincronizando Fornecedor...')
            await FornecedorService.Sincronizar()
          } else if (readOnly) {
            Alert.alert('Offline', 'Você está offline, os dados alterados deste fornecedores só estará disponível após a sincronização, você precisa de uma conexão para sincronizar esses dados!')
          } else {
            Alert.alert('Offline', 'Você está offline, este fornecedores só estará disponível após a sincronização, você precisa de uma conexão para sincronizar esses dados!')
          }

          if (!readOnly) {
            Alert.alert('Sucesso', 'Fornecedor cadastrado com sucesso!')
          } else {
            Alert.alert('Sucesso', 'Fornecedor editado com sucesso!')
          }
