...
import { CompraHistoricoService } from '../../../api/compraHistorico'
...
const HistoricoFornecedorScreen = ({ route }) => {
  const { defaultStyle, pickerItemColor } = useDefaultStyleSheet()
  const { fornecedorTempId } = route?.params
  ...
  useFocusEffect(useCallback(() => {
    ...
    const filtrar = async () => {
      try {
        ...
        if (isConnected) {
          await CompraHistoricoService.Sincronizar(dateEntrada, dateSaida, fornecedorTempId)
        }

        await HistoricoCompraDao.FiltroHistoricoFornecedor(dateEntrada, dateSaida, fornecedorTempId, 0).then(r => setListHistorico(r))
      } catch (error) {
    ...
    filtrar()
  }, [dateFim, dateInicio, fornecedorTempId]))
  ...
  const atualizarListagemOffset = useCallback(() => {
    ...
    HistoricoCompraDao.FiltroHistoricoFornecedor(dateEntrada, dateSaida, fornecedorTempId, offset).then(r => {
      if (!r.length) {
        setListagemCompleta(true)
        return
      }
      setListHistorico([...listHistorico, ...r])
    }).finally(() => setRefreshing(false))
  }, [dateFim, dateInicio, fornecedorTempId, listHistorico, offset])
...
export default HistoricoFornecedorScreen
