...
import ProdutoDAO, { TipoProduto } from '../../../db/ProdutoDao'
...
const QrCodeReader = () => {
  ...
  const { adicionarItem, selecionados, setProdutoSelecionadoIndex } = useContext(CompraContext)
  ...
  useFocusEffect(useCallback(() => refreshConfig(), [refreshConfig]))
  ...
  const consultarProduto = async (codigoInterno, tabelaPrecoId = null) => {
    try {
      const filtro = { produto: { codigoInterno }, tipoProduto: TipoProduto.Compra }
  ...
  const aumentarQuantidade = () => { setQuantidade(quantidade + 1); setQuantidadeStr((quantidade + 1).toFixed(item?.casasDecimais || 0)) }
  const diminuirQuantidade = () => { setQuantidade(quantidade - 1); setQuantidadeStr((quantidade - 1).toFixed(item?.casasDecimais || 0)) }
        ...
        <View style={[styles.mb15, { flexDirection: 'row' }]}>
          <View style={{ marginRight: 15 }}>
            ...
            <TextInputMask
              type="money"
              options={{
                precision: item?.casasDecimais || 0,
                separator: ',',
                delimiter: '.',
                unit: '',
                suffixUnit: ''
