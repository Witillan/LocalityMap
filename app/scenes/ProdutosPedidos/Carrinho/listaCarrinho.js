...
export default () => {
  ...
  const { selecionados, setSelecionados, produtoSelecionadoIndex, setProdutoSelecionadoIndex, setValidation, validation } = useContext(PedidoContext)
  ...
  const removerItemSelecionado = useCallback((index) => {
    setSelecionados(selecionados.filter((_, i) => index !== i))
  ...
  const adicionarTodos = useCallback(() => {
    let count = 0
    let firstError = null
  ...
  }, [navigation, selecionados, setProdutoSelecionadoIndex])

  useFocusEffect(useCallback(() => {
    const getConfig = async () => {
      setConfig(JSON.parse(await AsyncStorage.getItem('Configuracoes')))
    }
    getConfig()
  }, []))
  ...
  const renderItem = useCallback(({ item, index }) => <ItemCarrinho
    ...
    config={config}
  />, [config, editarItem, produtoSelecionadoIndex, removerItemSelecionado, setProdutoSelecionadoIndex, setValidation, validation])
