...
export class EstoqueService {
  static async Sincronizar () {
    ...
    const registrosComparacaoExclusao = await EstoqueDAO.GetListComparacaoRemocao()
    ...
    if (registrosComparacaoExclusao.length) {
      ...
      const responseObterRemovidos = await fetch(`${await getApiUrl()}/Estoques/ObterRegistrosRemovidos`, await getRequestOptions('POST', true, registrosComparacaoExclusao))
      ...
      await EstoqueDAO.Remove(removidos)
    }

    // Busca no DB os ids e timestamps de criação e alteração para verificar o que foi alterado/criado
    const registrosComparacaoAlteracao = await EstoqueDAO.GetListComparacaoAlteracoes()

    // Consulta na API os registros criados/alterados
    const responseObterAlterados = await fetch(`${await getApiUrl()}/Estoques/ObterRegistrosComparados`, await getRequestOptions('POST', true, registrosComparacaoAlteracao))
    ...
    await EstoqueDAO.AddOrReplaceFromAPI(criadosOuAlterados)
  }
