...
export class ProdutoService {
  static async Sincronizar () {
    ...
    const registrosComparacaoExclusao = await ProdutoDAO.GetListComparacaoRemocao()
    ...
    if (registrosComparacaoExclusao.length) {
      ...
      const responseObterRemovidos = await fetch(`${await getApiUrl()}/Produtos/ObterRegistrosRemovidos`, await getRequestOptions('POST', true, registrosComparacaoExclusao))
      ...
      if (!responseObterRemovidos.ok) {
        ...
        throw new Error(`Ocorreu um erro ${responseObterRemovidos.status} ao sincronizar os produtos no passo 1`)
      }
      ...
      await ProdutoDAO.Remove(removidos)
    }

    // Busca no DB os ids e timestamps de criação e alteração para verificar o que foi alterado/criado
    const registrosComparacaoAlteracao = await ProdutoDAO.GetListComparacaoAlteracoes()

    // Consulta na API os registros criados/alterados
    const responseObterAlterados = await fetch(`${await getApiUrl()}/Produtos/ObterRegistrosComparados`, await getRequestOptions('POST', true, registrosComparacaoAlteracao))

    if (!responseObterAlterados.ok) {
      await LogDAO.GravarLog(await responseObterAlterados.text())
      throw new Error(`Ocorreu um erro ${responseObterAlterados.status} ao sincronizar os produtos no passo 2`)
    }
    ...
    await ProdutoDAO.AddOrReplaceFromAPI(criadosOuAlterados)
  }
