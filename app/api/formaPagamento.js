...
export class FormaPagamentoService {
  static async Sincronizar () {
    ...
    const registrosComparacaoExclusao = await FormaPagamentoDAO.GetListComparacaoRemocao()
    ...
    if (registrosComparacaoExclusao.length) {
      ...
      const responseObterRemovidos = await fetch(`${await getApiUrl()}/FormasPagamento/ObterRegistrosRemovidos`, await getRequestOptions('POST', true, registrosComparacaoExclusao))
      ...
      if (!responseObterRemovidos.ok) {
        ...
        throw new Error(`Ocorreu um erro ${responseObterRemovidos.status} ao sincronizar as formas de pagamento no passo 1`)
      }
      ...
      await FormaPagamentoDAO.Remove(removidos)
    }

    // Busca no DB os ids e timestamps de criação e alteração para verificar o que foi alterado/criado
    const registrosComparacaoAlteracao = await FormaPagamentoDAO.GetListComparacaoAlteracoes()

    // Consulta na API os registros criados/alterados
    const responseObterAlterados = await fetch(`${await getApiUrl()}/FormasPagamento/ObterRegistrosComparados`, await getRequestOptions('POST', true, registrosComparacaoAlteracao))

    if (!responseObterAlterados.ok) {
      await LogDAO.GravarLog(await responseObterAlterados.text())
      throw new Error(`Ocorreu um erro ${responseObterAlterados.status} ao sincronizar as formas de pagamento no passo 2`)
    }
    ...
    await FormaPagamentoDAO.AddOrReplaceFromAPI(criadosOuAlterados)
  }
